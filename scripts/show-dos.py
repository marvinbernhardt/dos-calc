...
def _integral_from_dos(freq, dos):
    if len(freq) == 1:
        return np.mean(dos)
    else:
        return np.trapz(np.mean(dos, axis=0), x=freq)
...
def _plot_spectra(
    ...
    for h, moltype in enumerate(system["moltypes"]):
        ...
        filtered_spectra = {
            dos_name: dos
            for (dos_name, dos) in moltype["spectra"].items()
    ...
    if show_cross:
        for d, (cs_name, cs) in enumerate(system["cross_spectra"].items()):
            ...
            if temperature is None:
                ...
                print(
                    f"integral {cs_name}: "
                    f"{np.trapz(np.mean(cs, axis=0), freq):.4E} kJ/mol"
                )
            else:
                prefactor = 2 / K_GRO / temperature
                print(
                    f"integral {cs_name} / (1/2 k T): "
                    f"{np.trapz(np.mean(cs, axis=0), freq) * prefactor:.4E}"
                )
            (line,) = ax.plot(
                freq,
                prefactor * np.mean(cs, axis=0),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-c",
        "--components",
        help="show components DoS",
        dest="show_components",
        action="store_true",
    )
    parser.add_argument(
        "-x", "--cross", help="show cross DoS", dest="show_cross", action="store_true"
    )
    parser.add_argument(
        "-s",
        "--samples",
        help="show samples individually",
        dest="show_samples",
        action="store_true",
    )
    parser.add_argument(
        "-o", "--roto", help="show DoS_rotomega", dest="show_roto", action="store_true"
    )
    parser.add_argument(
        "-t",
        "--temperature",
        help="show integral normalized by 1/(kT)",
        dest="temperature",
        type=float,
        default=None,
    )
