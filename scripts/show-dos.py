    ...
    for h, moltype in enumerate(system['moltypes']):
        ...
        for d, (dos_name, dos) in enumerate(filtered_spectra.items()):
            ...
            if temperature is None:
                ...
                print(f"integral {dos_name}: {np.trapz(np.mean(dos, axis=0), freq) / K_GRO / temperature:.4E}")
            # plot
            if show_components:
    ...
    if show_cross:
        for d, (cs_name, cs) in enumerate(system['cross_spectra'].items()):
            ...
            if temperature is None:
                ...
                prefactor = 1 / K_GRO / temperature
                print(f"integral {cs_name}: {np.trapz(np.mean(cs, axis=0), freq) / K_GRO / temperature:.4E}")
