...
void get_frame_pos_box(CHFL_FRAME *frame, size_t natoms, float *pos,
                       float *box) {
...
void check_frame_natoms(CHFL_FRAME *frame, size_t natoms) {
    uint64_t natoms_traj = 0;
    chfl_vector3d *positions = NULL;
    chfl_frame_positions(frame, &positions, &natoms_traj);
    if (natoms_traj < natoms) {
        fprintf(stderr,
                "ERROR: The topology you give has more atoms than first "
                "frame of the trajectory/refconf\n");
        exit(1);
    } else if (natoms_traj > natoms) {
        fprintf(stderr,
                "WARNING: The topology you give has less atoms than first "
                "frame of the trajectory/refconf\n");
        fprintf(stderr, "         Some atoms are ignored in every frame\n");
    }
}

void check_frame_velocities(CHFL_FRAME *frame) {
...
void check_frame_orthorombic_box(CHFL_FRAME *frame, bool no_pbc) {
...
float get_traj_framelength(CHFL_TRAJECTORY *file, CHFL_FRAME *frame) {
    float framelength;
    double time0, time1;
    int result0, result1;
    // get time0
