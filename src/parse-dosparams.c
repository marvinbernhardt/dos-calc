  ...
  for (size_t h = 0; h < *nmoltypes; h++) {
    ...
    for (size_t j = 0; j < 4; j++) {
      ...
      if (((*moltypes_abc_indicators)[h][j] == -1) && (j == 0 || j == 2)) {
        fprintf(stderr,
                "ERROR: only the second and fourth abc indicator can be -1.\n");
        exit(1);
      }
    }
  }
  ...
  for (size_t d = 0; d < *ncross_spectra; d++) {
    ...
    for (size_t p = 0; p < (*cross_spectra_def)[d].ndof_pair_defs; p++) {
      ...
      if ((*cross_spectra_def)[d].dof_pair_defs[p].dofA_moltype >= *nmoltypes ||
        ...
        fprintf(stderr,
                "ERROR: moltype in cross spectrum higher than nmoltypes.\n");
        exit(1);
      }
      if ((*cross_spectra_def)[d].type == 'i' &&
          ((*cross_spectra_def)[d].dof_pair_defs[p].dofA_moltype !=
           (*cross_spectra_def)[d].dof_pair_defs[p].dofB_moltype)) {
        fprintf(stderr, "ERROR: For cross_spectrum type 'i' both moltypes have "
                        "to match.\n");
        exit(1);
      }
      // dof{A,B}_type
