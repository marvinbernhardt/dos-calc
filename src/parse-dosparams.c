        ...
        cross_spectrum_def **cross_spectra_def)
{
    ...
    *moltypes_abc_indicators = (int**) malloc(*nmoltypes * sizeof(int *));
    ...
    for (size_t h=0; h<*nmoltypes; h++)
    {
        ...
        json_parse_json_array(moltype_json, "abc_indicators", &abc_indicators_json);
    ...
    *cross_spectra_def = (cross_spectrum_def*) malloc(*ncross_spectra * sizeof(**cross_spectra_def));
    ...
    for (size_t d=0; d<*ncross_spectra; d++)
    {
        ...
        // name
        strcpy((*cross_spectra_def)[d].name, json_parse_string(cross_spectrum, "name"));
        // type
        ...
        (*cross_spectra_def)[d].dof_pair_defs = (dof_pair_def*) malloc((*cross_spectra_def)[d].ndof_pair_defs * sizeof(dof_pair_def));
        ...
        for (size_t p=0; p<(*cross_spectra_def)[d].ndof_pair_defs; p++)
        {
            ...
            // dof{A,B}_moletype
            (*cross_spectra_def)[d].dof_pair_defs[p].dofA_moltype = (size_t) json_parse_int(dofA, "moltype");
            (*cross_spectra_def)[d].dof_pair_defs[p].dofB_moltype = (size_t) json_parse_int(dofB, "moltype");
            if ((*cross_spectra_def)[d].dof_pair_defs[p].dofA_moltype >= *nmoltypes
             || (*cross_spectra_def)[d].dof_pair_defs[p].dofB_moltype >= *nmoltypes)
            {
                fprintf(stderr, "ERROR: moltype in cross spectrum higher than nmoltypes.\n");
                exit(1);
            }
            ...
            {
                fprintf(stderr, "ERROR: For cross_spectrum type 'i' both moltypes have to match.\n");
                exit(1);
            }
            // dof{A,B}_type
            (*cross_spectra_def)[d].dof_pair_defs[p].dofA_type = json_parse_char(dofA, "dof_type");
            (*cross_spectra_def)[d].dof_pair_defs[p].dofB_type = json_parse_char(dofB, "dof_type");
            ...
            (*cross_spectra_def)[d].dof_pair_defs[p].dofA_list = (size_t*) malloc(ndofA * sizeof(size_t));
            (*cross_spectra_def)[d].dof_pair_defs[p].dofB_list = (size_t*) malloc(ndofB * sizeof(size_t));
            for (size_t l=0; l<ndofA; l++)
            {
                cJSON *dof_indicator = cJSON_GetArrayItem(dofA_list, l);
                (*cross_spectra_def)[d].dof_pair_defs[p].dofA_list[l] = (size_t) json_parse_int_nokey(dof_indicator);
            }
            for (size_t l=0; l<ndofB; l++)
            {
                cJSON *dof_indicator = cJSON_GetArrayItem(dofB_list, l);
                (*cross_spectra_def)[d].dof_pair_defs[p].dofB_list[l] = (size_t) json_parse_int_nokey(dof_indicator);
            }
        }
    }
...
void free_dosparams_arrays(size_t nmoltypes,
        size_t **moltypes_nmols,
        size_t **moltypes_natomspermol,
        float ***moltypes_atommasses,
        char **moltypes_rot_treat,
