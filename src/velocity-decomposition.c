...
void decomposeVelocities(
    ...
    unsigned long nblocksteps, size_t natoms, size_t nmols, size_t nmoltypes,
    size_t *mol_firstatom, size_t *mol_natoms, size_t *mol_moltypenr,
    float **moltypes_atommasses, float *mol_mass, size_t *moltype_natomspermol,
    char *moltype_rot_treat, int **moltype_abc_indicators, bool no_pbc,
    // output
    float *mol_velocities_sqrt_m_trn, float *mol_omegas_sqrt_i_rot,
    float *atom_velocities_sqrt_m_vib, float *atom_velocities_sqrt_m_rot,
  ...
  static float *m_atommasses;
#pragma omp threadprivate(positions, velocities, positions_rel,                \
                          velocities_rot, m_atommasses)
...
#pragma omp parallel
  ...
  for (unsigned long t = 0; t < nblocksteps; t++) {
    ...
    // mol_vars, end_mol, after_abc, after_moi_save, after_eigen, bef_eigen,
    // up_ang_vel, pos_and_recomb, bef_recomb,
#endif

    float box[3];
    box[0] = block_box[3 * t + 0];
    box[1] = block_box[3 * t + 1];
    box[2] = block_box[3 * t + 2];
...
#pragma omp parallel for
    ...
    for (size_t i = 0; i < nmols; i++) {
      ...
      cblas_scopy(m_natoms, moltypes_atommasses[m_moltype], 1, m_atommasses, 1);
      // int *m_abc_indicators = moltype_abc_indicators[m_moltype];
      int m_abc_indicators[4] = {0, 0, 0, 0};
      memcpy(m_abc_indicators, moltype_abc_indicators[m_moltype],
             4 * sizeof(int));
      ...
      for (size_t j = 0; j < m_natoms; j++) {
        ...
        DPRINT("%8.3f%8.3f%8.3f%8.4f%8.4f%8.4f\n", positions[3 * j + 0],
               positions[3 * j + 1], positions[3 * j + 2],
               velocities[3 * j + 0], velocities[3 * j + 1],
               velocities[3 * j + 2]);
      }
      ...
      if (m_natoms == 1) {
        ...
        for (size_t dim = 0; dim < 3; dim++) {
          mol_velocities_sqrt_m_trn[3 * nblocksteps * i + nblocksteps * dim +
                                    t] = velocities[dim] * sqrt(m_mass);
      ...
      for (size_t dim = 0; dim < 3; dim++) {
        mol_velocity_trn[dim] =
            cblas_sdot(m_natoms, m_atommasses, 1, &velocities[0 + dim], 3);
        mol_velocity_trn[dim] /= m_mass;

        // output-array mol_velocities_sqrt_m_trn
        mol_velocities_sqrt_m_trn[3 * nblocksteps * i + nblocksteps * dim + t] =
      ...
      DPRINT("mol_velocity_trn: %8.4f%8.4f%8.4f\n", mol_velocity_trn[0],
             mol_velocity_trn[1], mol_velocity_trn[2]);
      ...
      for (size_t j = 0; j < m_natoms; j++) {
        ...
        DPRINT("pos_rel atom %zu: %8.4f%8.4f%8.4f\n", j,
               positions_rel[3 * j + 0], positions_rel[3 * j + 1],
               positions_rel[3 * j + 2]);
      }
      ...
      DPRINT("angular momentum: %8.4f%8.4f%8.4f\n", angular_momentum[0],
             angular_momentum[1], angular_momentum[2]);
      ...
      for (size_t j = 0; j < m_natoms; j++) {
        ...
        DPRINT("velocity_rot atom %zu: %8.4f%8.4f%8.4f\n", j,
               velocities_rot[3 * j + 0], velocities_rot[3 * j + 1],
               velocities_rot[3 * j + 2]);
      }
      ...
      for (size_t j = 0; j < m_natoms; j++) {
        ...
        cblas_saxpy(3, -1.0, &velocities_rot[3 * j], 1, velocity_vib, 1);
      ...
      if (m_rot_treat == 'l') {
        for (size_t dim = 0; dim < 3; dim++) {
          /*
          if (copysignf(1.0, angular_velocity[dim]) != copysignf(1.0,
          angular_momentum[dim]))
          {
              fprintf(stderr, "ERROR: angular velocity and angular_momentum
          ...
          */
          ...
          mol_omegas_sqrt_i_rot[3 * nblocksteps * i + nblocksteps * dim + t] =
              copysignf(
                  sqrt_neg_zero(angular_velocity[dim] * angular_momentum[dim]),
      ...
      // two numbers define a, two define b', c is cross product of a and b', b
      // is cross product of c and a vector from atom(first number) to
      // atom(second number) second number can be -1 for com c is always the
      // cross product

      DPRINT("abc indicators: (%d %d) (%d %d)\n", m_abc_indicators[0],
             m_abc_indicators[1], m_abc_indicators[2], m_abc_indicators[3]);
      ...
      if (m_rot_treat == 'a' || m_rot_treat == 'b') {
        ...
        DPRINT("angular velocity abc coord: %8.4f%8.4f%8.4f\n",
               angular_velocity_abc[0], angular_velocity_abc[1],
               angular_velocity_abc[2]);
        DPRINT("angular momentum abc coord: %8.4f%8.4f%8.4f\n",
               angular_momentum_abc[0], angular_momentum_abc[1],
               angular_momentum_abc[2]);
        ...
        cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, 3, 3, 3, 1.0,
                    abc_inv, 3, moi_tensor, 3, 0.0, temp_matrix, 3);
        DPRINT("temp_matrix:\n");
        ...
        cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, 3, 3, 3, 1.0,
                    temp_matrix, 3, abc, 3, 0.0, moi_tensor_abc, 3);
        DPRINT("moi_tensor_abc:\n");
        DPRINT("%f %f %f\n", moi_tensor_abc[0], moi_tensor_abc[1],
               moi_tensor_abc[2]);
        DPRINT("%f %f %f\n", moi_tensor_abc[3], moi_tensor_abc[4],
               moi_tensor_abc[5]);
        DPRINT("%f %f %f\n", moi_tensor_abc[6], moi_tensor_abc[7],
               moi_tensor_abc[8]);
        ...
        for (size_t dim = 0; dim < 3; dim++) {
          ...
          if (m_rot_treat == 'a') {
            mol_omegas_sqrt_i_rot[3 * nblocksteps * i + nblocksteps * dim + t] =
                angular_velocity_abc[dim] * sqrt(moi_tensor_abc[3 * dim + dim]);
          }
      ...
      for (size_t dim = 0; dim < 3; dim++) {
        angular_velocity_nc[dim] =
            cblas_sdot(3, angular_momentum, 1, &eigenvectors[dim], 3) /
            moments_of_inertia[dim];
      }
      DPRINT("angular velocity new coord: %8.4f%8.4f%8.4f\n",
             angular_velocity_nc[0], angular_velocity_nc[1],
             angular_velocity_nc[2]);
      // writing in output arrays the rotational velocities
      for (size_t dim = 0; dim < 3; dim++) {
        // 'f'ollowing the principal or abc axis; original method
        if (m_rot_treat == 'f') {
          mol_omegas_sqrt_i_rot[3 * nblocksteps * i + nblocksteps * dim + t] =
              angular_velocity_nc[dim] * sqrt(moments_of_inertia[dim]);
        }
      }
    ...
    for (size_t i = 0; i < nmols; i++) {
      DPRINT(
          "%5zu%5zu%5zu%8.4f%8.4f%8.4f%8.4f%8.4f%8.4f%8.4f\n", i, mol_natoms[i],
          mol_firstatom[i], mol_mass[i],
          mol_velocities_sqrt_m_trn[3 * nblocksteps * i + nblocksteps * 0 + t],
          mol_velocities_sqrt_m_trn[3 * nblocksteps * i + nblocksteps * 1 + t],
          mol_velocities_sqrt_m_trn[3 * nblocksteps * i + nblocksteps * 2 + t],
          mol_omegas_sqrt_i_rot[3 * nblocksteps * i + nblocksteps * 0 + t],
          mol_omegas_sqrt_i_rot[3 * nblocksteps * i + nblocksteps * 1 + t],
          mol_omegas_sqrt_i_rot[3 * nblocksteps * i + nblocksteps * 2 + t]);
    }
