  ...
  for (a = 0; a < 3; a++) {
    for (b = 0; b < 3; b++) {
      for (j = 0; j < mol_natoms; j++) {
        if (a == b) {
          tensor[3 * a + b] += atom_mass[j] * (pos1[3 * j + 0] * pos2[3 * j + 0] +
                                               pos1[3 * j + 1] * pos2[3 * j + 1] +
                                               pos1[3 * j + 2] * pos2[3 * j + 2]);
        }
        tensor[3 * a + b] -= atom_mass[j] * (pos1[3 * j + a] * pos2[3 * j + b]);
      }
    }
  }
}
...
lapack_int squareroot_of_matrix(float *A, unsigned N) {
  ...
  float C[N * N];
  cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans, N, N, N, 1.0, B, N, A, N,
              0.0, C, N);
