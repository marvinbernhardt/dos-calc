...
while True:
    dos_trn_file = Path(f"sample{sample}_dos_trn.txt")
    if not dos_trn_file.is_file():
        break
    ...
    for moltype in range(len(doses_to_show[0]['data'])):
        if frequencies:
            print(f"sample {sample}, integrals")
        else:
    ...
    for moltype in range(len(doses_to_show[0]['data'])):
        for dos in doses_to_show:
            if frequencies:
                plt.plot(frequencies, dos['data'][moltype], label=f"{moltype} {dos['name']}")
            else:
                plt.plot(dos['data'][moltype], label=f"{moltype} {dos['name']}")
